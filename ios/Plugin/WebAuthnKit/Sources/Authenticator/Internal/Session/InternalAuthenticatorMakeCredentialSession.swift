...
public class InternalAuthenticatorMakeCredentialSession : AuthenticatorMakeCredentialSession {
    ...
    private func getKeyName() throws -> Promise<String>  {
        guard let customKeyName = self.setting.customKeyName else {
            throw WAKError.unknown
        }
        return Promise<String>{ $0.fulfill(customKeyName) }
    }
    ...
    public func makeCredential(
        ...
        firstly {
            ...
            keySupport.removeKeyPair(label: credSource.keyLabel)
            ...
            if let err = error as? WAKError {
                ...
                case .authenticationFailed:
                    self.stop(by: .authenticationFailed)
                case .userCancel:
                    self.stop(by: .userCancel)
                case .userFallback:
                    self.stop(by: .userFallback)
                case .systemCancel:
                    self.stop(by: .systemCancel)
                case .passcodeNotSet:
                    self.stop(by: .passcodeNotSet)
                case .touchIDNotAvailable:
                    self.stop(by: .touchIDNotAvailable)
                case .touchIDNotEnrolled:
                    self.stop(by: .touchIDNotEnrolled)
                case .touchIDLockout:
                    self.stop(by: .touchIDLockout)
                case .appCancel:
                    self.stop(by: .appCancel)
                case .invalidContext:
                    self.stop(by: .invalidContext)
                case .notInteractive:
                    self.stop(by: .notInteractive)
                case .biometryLockout:
                    self.stop(by: .biometryLockout)
                case .biometryNotAvailable:
                    self.stop(by: .biometryNotAvailable)
                case .biometryNotEnrolled:
                    self.stop(by: .biometryNotEnrolled)
                @unknown default:
                    self.stop(by: .unknown)
                }
            } else {
