...
public protocol KeySupport {
    ...
    func sign(data: [UInt8], label: String) throws -> Optional<[UInt8]>
    func removeKeyPair(label: String)
}
...
public class ECDSAKeySupport : KeySupport {
    ...
    private func createPair(label: String) -> EllipticCurveKeyPair.Manager {
        ...
        let config = EllipticCurveKeyPair.Config(
            ...
            token:                   EllipticCurveKeyPair.Token.secureEnclave
        )
        return EllipticCurveKeyPair.Manager(config: config)
    }
    
    public func sign(data: [UInt8], label: String) throws -> Optional<[UInt8]> {
        do {
            let pair = self.createPair(label: label)
            let signature = try pair.sign(Data(bytes: data), hash: .sha256)
            return signature.bytes
        } catch EllipticCurveKeyPair.Error.underlying(_, let underlying) {
            throw underlying
        } catch EllipticCurveKeyPair.Error.authentication(let authenticationError) {
    ...
    public func removeKeyPair(label: String) {
        WAKLogger.debug("<ECDSAKeySupport> removeKeyPair")
        do {
            let pair = self.createPair(label: label)
            try pair.deleteKeyPair()
        } catch let error {
            WAKLogger.debug("<ECDSAKeySupport> failed to remove key-pair: \(error)")
        }
    }
}
